...
@objc(LaVieKeyIMController)
class LaVieKeyIMController: IMKInputController {
    ...
    private var settings: LaVieKeyIMSettings!
    ...
    override init!(server: IMKServer!, delegate: Any!, client inputClient: Any!) {
        ...
        // Listen for settings changes from LaVieKey app
        DistributedNotificationCenter.default().addObserver(
            self,
            selector: #selector(handleSettingsChanged),
            name: Notification.Name("LaVieKey.settingsDidChange"),
            object: nil
        )

        NSLog("LaVieKeyIMController: Initialized")
    }
    ...
    /// Handle settings changed notification from LaVieKey app
    ...
    override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
        ...
        // Use centralized logic from VNEngine to ensure consistency with LaVieKey main app
    ...
    private func handleResult(_ result: VNEngine.ProcessResult, client: IMKTextInput) {
        ...
        // User can control marked text mode via LaVieKey settings
    ...
    override func activateServer(_ sender: Any!) {
        ...
        // Log version to debug window when LaVieKeyIM is activated
        let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "?"
        let build = Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "?"
        IMKitDebugger.shared.log("LaVieKeyIM v\(version) (build \(build)) activated", category: "ACTIVATE")
        
        NSLog("LaVieKeyIMController: Activated")
    }
    ...
    override func deactivateServer(_ sender: Any!) {
        ...
        NSLog("LaVieKeyIMController: Deactivated")
    }
    ...
    override func menu() -> NSMenu! {
        ...
        // Open LaVieKey settings
        let settingsItem = NSMenuItem(
            title: "Mở LaVieKey Settings...",
            action: #selector(openLaVieKeySettings),
            keyEquivalent: ""
        )
    ...
    @objc private func toggleVietnamese() {
        ...
        currentWordLength = 0
        markedTextStartLocation = NSNotFound
        NSLog("LaVieKeyIMController: Vietnamese = \(isVietnameseEnabled)")
    }
    
    @objc private func openLaVieKeySettings() {
        // Use URL scheme to open LaVieKey settings directly
        // This will open the settings window, not just the app
        if let url = URL(string: "laviekey://settings") {
            NSWorkspace.shared.open(url)
            NSLog("LaVieKeyIMController: Opened laviekey://settings")
        } else {
            // Fallback: Just launch the app
            let process = Process()
            process.executableURL = URL(fileURLWithPath: "/usr/bin/open")
            process.arguments = ["-b", "lavie.nguyen.LaVieKey"]
            
            do {
                try process.run()
                NSLog("LaVieKeyIMController: Launched LaVieKey app (fallback)")
            } catch {
                NSLog("LaVieKeyIMController: Failed to launch LaVieKey: \(error)")
            }
        }
    }
}
...
/// Settings wrapper for LaVieKeyIM
/// ARCHITECTURE: Uses plist file directly for reliable sync with LaVieKey app
class LaVieKeyIMSettings {
    
    /// App Group identifier
    /// Note: macOS Sequoia+ requires TeamID prefix for native apps outside App Store
    private let appGroup = "98NNYCQ84U.lavie.nguyen.inputmethod.LaVieKey"
    ...
    func reload() {
        ...
        if let method = InputMethod(rawValue: readInt(forKey: "LaVieKey.inputMethod")) {
            inputMethod = method
        ...
        // Code Table
        if let table = CodeTable(rawValue: readInt(forKey: "LaVieKey.codeTable")) {
            codeTable = table
        }
        
        // Boolean settings
        modernStyle = readBool(forKey: "LaVieKey.modernStyle")
        spellCheckEnabled = readBool(forKey: "LaVieKey.spellCheckEnabled")

        quickTelexEnabled = readBool(forKey: "LaVieKey.quickTelexEnabled", defaultValue: true)
        quickStartConsonantEnabled = readBool(forKey: "LaVieKey.quickStartConsonantEnabled")
        quickEndConsonantEnabled = readBool(forKey: "LaVieKey.quickEndConsonantEnabled")
        restoreIfWrongSpelling = readBool(forKey: "LaVieKey.restoreIfWrongSpelling", defaultValue: true)
        upperCaseFirstChar = readBool(forKey: "LaVieKey.upperCaseFirstChar")
        ...
        useMarkedText = readBool(forKey: "LaVieKey.imkitUseMarkedText", defaultValue: true)
        ...
        // Debug Mode - controls whether LaVieKeyIM writes to ~/LaVieKey_Debug.log
        debugModeEnabled = readBool(forKey: "LaVieKey.debugModeEnabled")
    }
