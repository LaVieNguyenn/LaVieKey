...
//  LaVieKey
//
//  Manages macOS Input Sources and auto-enables/disables LaVieKey based on configuration
//
...
/// Configuration for auto-enabling LaVieKey per input source
struct InputSourceConfig: Codable {
    var enabledInputSources: [String: Bool] = [:]  // Input Source ID -> enabled state

    /// Check if LaVieKey should be enabled for a given input source
    func isLaVieKeyEnabled(for inputSourceID: String) -> Bool {
...
class InputSourceManager {
    ...
    @objc private func inputSourceDidChange(_ notification: Notification) {
        ...
        // For LaVieKey/OpenKey itself - always enable Vietnamese
        let shouldEnable: Bool
        if Self.isLaVieKeyInputSource(newSource) {
            shouldEnable = true
        } else {
            // Check if LaVieKey should be enabled for this input source
            shouldEnable = config.isLaVieKeyEnabled(for: newSource.id)
        }

        debugLogCallback?("Input Source: \(previousSource) → \(newSource.displayName) [LaVieKey: \(shouldEnable ? "ON" : "OFF")]")
    ...
    func setEnabled(_ enabled: Bool, for inputSourceID: String) {
        ...
        if currentInputSource?.id == inputSourceID {
            if let source = currentInputSource {
                debugLogCallback?("Input Source '\(source.displayName)' → LaVieKey: \(enabled ? "ON" : "OFF") (applied now)")
    ...
    func isEnabled(for inputSourceID: String) -> Bool {
        return config.isLaVieKeyEnabled(for: inputSourceID)
    }
    ...
    static func getAllInputSources() -> [InputSourceInfo] {
        ...
        // Filter out LaVieKey/OpenKey itself from the list
        return uniqueSources.filter { !isLaVieKeyInputSource($0) }
    }
    ...
    /// Check if an input source is LaVieKey/OpenKey itself
    static func isLaVieKeyInputSource(_ source: InputSourceInfo) -> Bool {
        let lowercaseID = source.id.lowercased()
        let lowercaseName = source.name.lowercased()

        // Match both the LaVieKey bundle id (lavie.nguyen.inputmethod.LaVieKey)
        // and legacy XKey installs. The old ["xkey"] pattern silently stopped
        // matching after the bundle-id rebrand.
        let patterns = ["laviekey", "xkey"]
...
extension Notification.Name {
    ...
    static let inputSourceDidChange = Notification.Name("LaVieKey.inputSourceDidChange")
}
