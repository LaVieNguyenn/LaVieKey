...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var switchLaVieKeyHotkeyMonitor: Any?
    private var switchLaVieKeyGlobalHotkeyMonitor: Any?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
            ...
            if let bundleId = Bundle.main.bundleIdentifier {
                ...
                if !otherInstances.isEmpty {
                    ...
                    for oldInstance in otherInstances {
                        NSLog("[LaVieKey] Terminating old instance (PID: %d)", oldInstance.processIdentifier)
        ...
        // Reload rules into the engine when the rules store is rewritten. Matching reads
        // AppBehaviorDetector.customRules live, so without this an external edit/delete would not
        // take effect until the next app launch. clearCache() drops any per-app behavior memoization.
        ...
        // Check and update LaVieKeyIM if needed (on app startup)
        checkLaVieKeyIMUpdate()
        ...
        debugWindowController?.updateStatus("LaVieKey started successfully")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        // Remove switch LaVieKey hotkey monitors
        if let monitor = switchLaVieKeyHotkeyMonitor {
            NSEvent.removeMonitor(monitor)
        }
        if let monitor = switchLaVieKeyGlobalHotkeyMonitor {
            NSEvent.removeMonitor(monitor)
        }
    ...
    /// Handle URL scheme: laviekey://settings opens preferences
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls {
            guard url.scheme == "laviekey" else { continue }
    ...
    private func setupKeyboardHandling() {
        ...
        // Check if current input source is LaVieKeyIM
        ...
        if let currentSource = InputSourceManager.getCurrentInputSource(),
           InputSourceManager.isLaVieKeyInputSource(currentSource) {
            debugWindowController?.logEvent("  Current input source is LaVieKeyIM - event tap will NOT start")
            debugWindowController?.logEvent("     Event tap will start automatically when switching away from LaVieKeyIM")
            return
        }
    ...
    private func applyPreferences(_ preferences: Preferences) {
        ...
        // Update switch LaVieKey hotkey
        setupSwitchLaVieKeyHotkey(with: preferences.switchToLaVieKeyHotkey)
    ...
    private func showPermissionAlert() {
        ...
        LaVieKey needs accessibility permission to function as a Vietnamese input method.
        
        Please grant permission in System Settings > Privacy & Security > Accessibility.
        
        After granting permission, LaVieKey will automatically start working.
        """
    ...
    private func setupReadWordHotkey() {
        ...
        // Local monitor - catches hotkey when LaVieKey app is focused
    ...
    // State tracking for modifier-only switch LaVieKey hotkey
    private var switchLaVieKeyModifierState: (targetReached: Bool, hasTriggered: Bool) = (false, false)
    private var switchLaVieKeyFlagsMonitor: Any?
    private var switchLaVieKeyGlobalFlagsMonitor: Any?
    
    private func setupSwitchLaVieKeyHotkey(with hotkey: Hotkey?) {
        // Remove existing monitors
        if let monitor = switchLaVieKeyHotkeyMonitor {
            NSEvent.removeMonitor(monitor)
            switchLaVieKeyHotkeyMonitor = nil
        }
        if let monitor = switchLaVieKeyGlobalHotkeyMonitor {
            NSEvent.removeMonitor(monitor)
            switchLaVieKeyGlobalHotkeyMonitor = nil
        }
        if let monitor = switchLaVieKeyFlagsMonitor {
            NSEvent.removeMonitor(monitor)
            switchLaVieKeyFlagsMonitor = nil
        }
        if let monitor = switchLaVieKeyGlobalFlagsMonitor {
            NSEvent.removeMonitor(monitor)
            switchLaVieKeyGlobalFlagsMonitor = nil
        }
        
        // Reset modifier state
        switchLaVieKeyModifierState = (false, false)
        
        guard let hotkey = hotkey else {
            debugWindowController?.logEvent("  Switch LaVieKey hotkey disabled")
            return
        }
        ...
        let performToggle: () -> Void = { [weak self] in
            DispatchQueue.main.async {
                ...
                let wasLaVieKey = InputSourceSwitcher.shared.isLaVieKeyActive
                let action = wasLaVieKey ? "LaVieKey → ABC" : "ABC → LaVieKey"

                // Perform toggle
                let success = InputSourceSwitcher.shared.toggleLaVieKey()
        ...
        if hotkey.isModifierOnly {
            ...
            let handleFlagsChanged: (NSEvent) -> Void = { [weak self] event in
                ...
                if hasAllRequiredModifiers {
                    ...
                    if !self.switchLaVieKeyModifierState.targetReached {
                        self.switchLaVieKeyModifierState.targetReached = true
                        self.switchLaVieKeyModifierState.hasTriggered = false
                        self.debugWindowController?.logEvent("  → Switch LaVieKey target modifiers REACHED: \(hotkey.displayString)")
                    }
                } else {
                    // Modifiers changed (released)
                    if self.switchLaVieKeyModifierState.targetReached && !self.switchLaVieKeyModifierState.hasTriggered {
                        // Was holding target modifiers, now released - TRIGGER!
                        self.switchLaVieKeyModifierState.hasTriggered = true
                        self.debugWindowController?.logEvent("  → SWITCH LAVIEKEY MODIFIER-ONLY HOTKEY TRIGGERED on release: \(hotkey.displayString)")
                        performToggle()
                    }
                    // Reset state
                    self.switchLaVieKeyModifierState.targetReached = false
                }
            }
            
            // Global monitor for flagsChanged
            switchLaVieKeyGlobalFlagsMonitor = NSEvent.addGlobalMonitorForEvents(matching: .flagsChanged) { event in
                handleFlagsChanged(event)
            }
            
            // Local monitor for flagsChanged
            switchLaVieKeyFlagsMonitor = NSEvent.addLocalMonitorForEvents(matching: .flagsChanged) { event in
                handleFlagsChanged(event)
            ...
            switchLaVieKeyGlobalHotkeyMonitor = NSEvent.addGlobalMonitorForEvents(matching: .keyDown) { [weak self] _ in
                // If user presses any key while holding modifiers, cancel the modifier-only hotkey
                if self?.switchLaVieKeyModifierState.targetReached == true {
                    self?.debugWindowController?.logEvent("  → Key pressed while holding modifiers - canceling switch LaVieKey modifier-only hotkey")
                    self?.switchLaVieKeyModifierState.targetReached = false
                    self?.switchLaVieKeyModifierState.hasTriggered = true  // Prevent trigger on release
                }
            }
            
            switchLaVieKeyHotkeyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
                // If user presses any key while holding modifiers, cancel the modifier-only hotkey
                if self?.switchLaVieKeyModifierState.targetReached == true {
                    self?.debugWindowController?.logEvent("  → Key pressed while holding modifiers - canceling switch LaVieKey modifier-only hotkey")
                    self?.switchLaVieKeyModifierState.targetReached = false
            ...
            debugWindowController?.logEvent("Switch LaVieKey hotkey (modifier-only): \(hotkey.displayString)")
        } else {
            // Regular hotkey (e.g., Cmd+Shift+V)
            
            // Global monitor - catches hotkey in ALL apps
            switchLaVieKeyGlobalHotkeyMonitor = NSEvent.addGlobalMonitorForEvents(matching: .keyDown) { event in
                if event.keyCode == keyCode && checkModifiers(event) {
                    performToggle()
                }
            }
            ...
            // Local monitor - catches hotkey when LaVieKey app is focused
            switchLaVieKeyHotkeyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
                if event.keyCode == keyCode && checkModifiers(event) {
                    performToggle()
                    // Return nil to consume the event
                    return nil
            ...
            debugWindowController?.logEvent("Switch LaVieKey hotkey: \(hotkey.displayString)")
        }
    }
    ...
    private func handleSmartSwitch(notification: Notification) {
        ...
        if inputSourceOverride.hasTarget, let targetId = inputSourceOverride.inputSourceId {
            ...
            // NOTE: This MUST run even when current IS is disabled (e.g., LaVieKey IS from previous rule).
        ...
        // returns the restored IS (not the old rule-controlled LaVieKey IS).
    ...
    private func setupInputSourceManager() {
        ...
        if let currentSource = InputSourceManager.getCurrentInputSource() {
            ...
            debugWindowController?.logEvent("   Should enable LaVieKey: \(shouldEnable)")
    ...
    private func handleInputSourceChange(source: InputSourceInfo, shouldEnable: Bool) {
        // Check if this is LaVieKeyIM input source
        let isLaVieKeyIM = InputSourceManager.isLaVieKeyInputSource(source)

        if isLaVieKeyIM {
            // Switched TO LaVieKeyIM - suspend CGEvent tap to let IMKit handle events
            debugWindowController?.logEvent("🔑 Switched to LaVieKeyIM - suspending CGEvent tap")
            eventTapManager?.suspend()

            // Force DISABLE LaVieKey main app's Vietnamese engine
            // LaVieKeyIM handles Vietnamese typing itself, so LaVieKey main app shows "E" (engine off)
            ...
            // If not (e.g., started with LaVieKeyIM active), start it now
            guard let manager = eventTapManager else { return }
            ...
            // `shouldEnable` comes from global IS config (InputSourceConfig.isLaVieKeyEnabled).
    ...
    // MARK: - LaVieKeyIM Auto-Update

    private func checkLaVieKeyIMUpdate() {
        // Connect LaVieKeyIMUpdateManager debug logging
        LaVieKeyIMUpdateManager.shared.debugLogCallback = { [weak self] message in
            self?.debugWindowController?.logEvent(message)
        }
        
        // Always install bundled LaVieKeyIM after a short delay
        // This ensures LaVieKeyIM is always in sync with LaVieKey app
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            LaVieKeyIMUpdateManager.shared.installBundledLaVieKeyIM(showNotification: false)
        }
    }
    ...
    /// making LaVieKey and all third-party input methods completely non-functional.
    ...
    private func checkAndWarnSecureInput() -> Bool {
        ...
        if isSecure {
            ...
            if !samePID || (now - lastSecureInputLogTime) > 30.0 {
                ...
                debugWindowController?.logEvent("⚠️ Secure Input đang BẬT bởi '\(name)' — LaVieKey không thể nhận phím!")
            ...
            if lastSecureInputPID != 0 {
                ...
                debugWindowController?.logEvent("✅ Secure Input đã TẮT — LaVieKey hoạt động bình thường")
