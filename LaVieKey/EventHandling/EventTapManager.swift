...
class EventTapManager {
    ...
    private func eventCallback(
        ...
        // even when off-console, otherwise LaVieKey will stop working when the user returns.
        ...
        // - LaVieKey hotkeys firing in the wrong session
        ...
        // CRITICAL: Skip events injected by LaVieKey itself
        ...
        if event.getIntegerValueField(.eventSourceUserData) == kLaVieKeyEventMarker {
            return Unmanaged.passUnretained(event)
        }
        ...
        if isHIDTapActive {
            event.setIntegerValueField(.eventSourceUserData, value: kLaVieKeyHIDSeenMarker)
        }
        ...
        // Since LaVieKey only processes keyDown for Vietnamese input, keyUp can bypass safely.
        if type == .keyUp {
            return Unmanaged.passUnretained(event)
        }
    ...
    private func sessionEventCallback(
        ...
        // Skip events injected by LaVieKey itself
        let userData = event.getIntegerValueField(.eventSourceUserData)
        if userData == kLaVieKeyEventMarker {
            return Unmanaged.passUnretained(event)
        }
        ...
        if userData == kLaVieKeyHIDSeenMarker {
            // Restore clean state before passing to downstream apps/utilities
            // This prevents kLaVieKeyHIDSeenMarker from leaking to other keyboard tools
        ...
        // Pass through keyUp events (LaVieKey only processes keyDown)
        if type == .keyUp {
            return Unmanaged.passUnretained(event)
        }
    ...
    /// This makes LaVieKey and ALL third-party input methods completely non-functional.
    ///
    ...
    ///
    /// Problem: When LaVieKey uses `.cghidEventTap` (HID level), the event tap intercepts
    /// ALL keyboard events system-wide, including those from other user sessions.
    /// This causes LaVieKey to "steal" keystrokes from other users and inject Vietnamese
