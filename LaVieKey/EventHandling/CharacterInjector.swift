...
// MARK: - Event Marker
// Used to identify events injected by LaVieKey - prevents re-processing by event tap
// This is critical for avoiding race conditions in terminal apps
let kLaVieKeyEventMarker: Int64 = 0x584B4559  // "XKEY" in hex
let kLaVieKeyHIDSeenMarker: Int64 = 0x584B4849  // "XKHI" in hex - marks events seen by HID tap
...
class CharacterInjector {
        ...
        for (segmentIndex, segment) in segments.enumerated() {
            ...
            case .text:
                ...
                while offset < utf16.count {
                    ...
                    // Mark as LaVieKey-injected event to prevent re-processing by event tap
                    keyDown.setIntegerValueField(.eventSourceUserData, value: kLaVieKeyEventMarker)
                    keyUp.setIntegerValueField(.eventSourceUserData, value: kLaVieKeyEventMarker)
    ...
    private func sendTextViaPaste(_ text: String, proxy: CGEventTapProxy, config: PasteConfig = PasteConfig()) {
        ...
        let postPaste: (CGEvent) -> Void = { event in
            event.setIntegerValueField(.eventSourceUserData, value: kLaVieKeyEventMarker)
