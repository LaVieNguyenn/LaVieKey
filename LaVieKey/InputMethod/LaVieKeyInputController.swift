...
///
/// Usage:
/// 1. Build as separate LaVieKeyIM.app bundle
/// 2. Install to ~/Library/Input Methods/
/// 3. Enable in System Settings → Keyboard → Input Sources
/// 4. Select "LaVieKey Vietnamese" as input source
@available(macOS 10.15, *)
class LaVieKeyInputController: IMKInputController {
    
    // MARK: - Properties
    
    /// Vietnamese processing engine (shared with main LaVieKey)
    ...
    override init!(server: IMKServer!, delegate: Any!, client inputClient: Any!) {
        ...
        // Load settings (could sync with main LaVieKey app)
        loadSettings()
    }
    ...
    private func loadSettings() {
        ...
        // This allows settings to be shared between LaVieKey and LaVieKeyIM
        // Note: macOS Sequoia+ requires TeamID prefix for native apps outside App Store
        let appGroup = "98NNYCQ84U.lavie.nguyen.inputmethod.LaVieKey"
        ...
        if let methodRaw = dict["LaVieKey.inputMethod"] as? Int,
           let method = InputMethod(rawValue: methodRaw) {
            inputMethod = method
        }

        if let tableRaw = dict["LaVieKey.codeTable"] as? Int,
           let table = CodeTable(rawValue: tableRaw) {
            codeTable = table
        }

        if let markedText = dict["LaVieKey.imkitUseMarkedText"] as? Bool {
            useMarkedText = markedText
        } else if let markedText = dict["LaVieKey.imkitUseMarkedText"] as? Int {
...
/// Main entry point for LaVieKeyIM.app bundle
/// This would be used when building LaVieKey as a standalone Input Method
@available(macOS 10.15, *)
class LaVieKeyIMAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        server = IMKServer(
            name: "LaVieKeyIM_Connection",
            bundleIdentifier: Bundle.main.bundleIdentifier!
        )
        
        NSLog("LaVieKeyIM: Input Method server started")
    }
