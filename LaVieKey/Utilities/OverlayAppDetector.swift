...
class OverlayAppDetector {
    ...
    private func detectOverlayViaAXAttributes() -> String? {
        ...
        // Check owning process FIRST (most robust). Spotlight runs as its own
        // process (com.apple.Spotlight) whose only UI is the search overlay, so
        // process identity alone is conclusive. The attribute patterns below are
        // version-fragile: macOS 26 (Tahoe) redesigned Spotlight and its search
        // field no longer exposes the old "SpotlightSearchField" identifier /
        // "Spotlight Search" placeholder, which made detection silently fail —
        // injection then kept the previous app's method and Spotlight swallowed
        // the synthesized backspaces ("window" → "ưwindoơow").
        // Raycast/Alfred intentionally stay on attribute matching: their
        // processes also own ordinary windows (preferences, etc.) where
        // overlay treatment would be wrong.
        var ownerPid: pid_t = 0
        if AXUIElementGetPid(axElement, &ownerPid) == .success, ownerPid > 0,
           NSRunningApplication(processIdentifier: ownerPid)?.bundleIdentifier == "com.apple.Spotlight" {
            return "Spotlight"
        }
    ...
    // - Requires Accessibility permission (which LaVieKey already needs)
