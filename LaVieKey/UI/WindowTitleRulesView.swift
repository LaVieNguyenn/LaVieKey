...
struct WindowTitleRulesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Cho phép hiệu chỉnh LaVieKey Engine xử lý Tiếng Việt theo từng ứng dụng cụ thể (ví dụ: Google Docs trong Safari)")
                        .font(.caption)
        ...
        .fileExporter(
            ...
            contentType: .json,
            defaultFilename: "laviekey_window_rules.json"
        ) { result in
...
struct AddRuleSheet: View {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if overrideInputSource {
                            if availableInputSources.isEmpty {
                                ...
                                Text("LaVieKey sẽ tự động chuyển sang Input Source đã chọn.")
                                    .font(.caption2)
                                    .foregroundColor(.secondary)
                                    .padding(.leading, 20)
                            }
    ...
    private func loadAvailableInputSources() {
        ...
        // Add LaVieKey at the beginning (it's filtered out by getAllInputSources)
        let lavieKeySource = InputSourceInfo(
            id: InputSourceSwitcher.lavieKeyIMBundleId,
            name: "LaVieKey"
        )
        sources.insert(lavieKeySource, at: 0)
...
class WindowTitleRulesViewModel: ObservableObject {
    ...
    /// Observer that refreshes the list when the rules store is rewritten externally.
    ...
    init() {
        ...
        // Refresh the Settings list when the rules store is rewritten externally. Reload the store
    ...
    private func refreshDetectionOnly() {
        ...
        if let bundleId = detector.getCurrentBundleId() {
            let isLaVieKey = bundleId == "lavie.nguyen.LaVieKey" || bundleId == "lavie.nguyen.LaVieKey.debug"
    ...
    func refresh() {
        ...
        // Skip updating if current app is LaVieKey itself (user is viewing settings)
        // This keeps the previously detected app info visible
        if let bundleId = detector.getCurrentBundleId() {
            let isLaVieKey = bundleId == "lavie.nguyen.LaVieKey" || bundleId == "lavie.nguyen.LaVieKey.debug"
            
            if !isLaVieKey {
                // Only update when not LaVieKey
                currentAppName = NSWorkspace.shared.frontmostApplication?.localizedName ?? bundleId
                currentBundleId = bundleId
                currentWindowTitle = detector.getCachedWindowTitle()
            ...
            // If LaVieKey, keep previous values (don't update)
    ...
    func updateRule(_ rule: WindowTitleRule, originalId: UUID?) {
        if let originalId = originalId, originalId != rule.id {
            // Identity changed (defensive — saveRule preserves the id, so this is rare): remove the
            // old id, then add the new entry. Avoids a duplicate.
            AppBehaviorDetector.shared.removeCustomRule(id: originalId)
            AppBehaviorDetector.shared.addCustomRule(rule)
        } else {
            // Same id: update in place. Preserves list order and identity (UUID), so the edit does
