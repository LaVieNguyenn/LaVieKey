...
extension Notification.Name {
    static let macrosDidChange = Notification.Name("LaVieKey.macrosDidChange")
}
...
class MacroManagementViewModel: ObservableObject {
    ...
    init() {
        // Keep an open Macro tab in sync with the store. When an external change applies a remote
    ...
    func exportMacros() {
        ...
        do {
            var lines = ["# LaVieKey Macros", "# Format: shortcut=replacement (use \\n for newlines)", ""]
    ...
    /// System Settings > Keyboard > Text Replacements. LaVieKey never writes the
    ...
    func exportToTextReplacementsPlist() {
        ...
        panel.nameFieldStringValue = "LaVieKey Text Replacements.plist"
...
/// on macOS 26.5.1). LaVieKey only generates this file; the user performs the import.
