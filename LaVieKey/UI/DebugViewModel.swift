...
class DebugViewModel: ObservableObject {
    ...
    /// Last external app PID (not LaVieKey) - used for Force Accessibility targeting
    ...
    private let writeQueue = DispatchQueue(label: "com.laviekey.debuglog.write", qos: .utility)
    ...
    init() {
        ...
        // Listen for debug logs from LaVieKeyIM
    ...
    private func initializeLogFile() {
        ...
        let versionString = "LaVieKey v\(AppVersion.current) (\(AppVersion.build))"
    ...
    private func setupIMKitDebugListener() {
        DistributedNotificationCenter.default().addObserver(
            ...
            // Fallback to userInfo for LaVieKeyIM messages with source
    ...
    private func pollExternalAppInfo() {
        ...
        // Track external app (any app that's not LaVieKey)
        // This is used for Force Accessibility to target the correct app
        let myBundleID = Bundle.main.bundleIdentifier ?? "lavie.nguyen.LaVieKey"
    ...
    /// Enable AXManualAccessibility for the last focused external app (not LaVieKey)
    func enableForceAccessibility() {
        // Use the last tracked external app (not LaVieKey itself)
    ...
    /// This is more reliable than backspaces because LaVieKey engine won't process
    ...
    private func simulateKeyPress(_ char: Character) {
        ...
        if let keyDown = CGEvent(keyboardEventSource: source, virtualKey: keyCode, keyDown: true),
           let keyUp = CGEvent(keyboardEventSource: source, virtualKey: keyCode, keyDown: false) {
            ...
            // Do NOT set Unicode string - let the system (and LaVieKey EventTap) handle it
            // If we set Unicode string, EventTap might not process it correctly
            
            // Do NOT set the LaVieKey marker - we want EventTap to process these as real keystrokes
