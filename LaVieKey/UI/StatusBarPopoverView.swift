...
struct StatusBarPopoverView: View {
        ...
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 2) {
                Text("Remote Session Tap")
                    .font(.system(size: 13, weight: .semibold))
                Text("Hỗ trợ gõ tiếng Việt khi máy A remote (tắt LaVieKey) -> máy B được remote (bật LaVieKey + bật tính năng này)")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
                    .lineLimit(nil)
    ...
    private var footerSection: some View {
        VStack(spacing: 1) {
            ...
            ActionRow(title: "Thoát LaVieKey", icon: "power", shortcut: "⌘Q") {
                viewModel.quit()
            }
        }
        .menuCardStyle()
